...
sudo_call() {
    ...
    OUTPUT="$(junod tx wasm submit-proposal sudo-contract $POC_CONTRACT_ADDRESS $1 --from $DEV_ACCOUNT --node $RPC_URL --gas-prices 0.025ujunox --gas auto --gas-adjustment 1.3 -y --output json --chain-id $CHAIN_ID --title "sudo-execute"  --summary "post_results" --deposit "$2"ujunox)"
    echo $OUTPUT
...
migrate_call() {
    ...
    echo "id" $2
    echo "msg" $3
    OUTPUT="$(junod tx wasm migrate $1 $2 $3 --node $RPC_URL --output json --from $DEV_ACCOUNT --node $RPC_URL --gas-prices 0.025ujunox --gas auto --gas-adjustment 1.3 -y --output json --chain-id $CHAIN_ID)"
    echo $OUTPUT
...
smart_query() {
    ...
    OUTPUT="$(junod query wasm contract-state smart $POC_CONTRACT_ADDRESS $1 --node $RPC_URL --output json)"
    echo $OUTPUT
...
raw_query_contract() {
    ...
    OUTPUT="$(junod query wasm contract-state raw $POC_CONTRACT_ADDRESS 636F6E74726163745F696E666F --node $RPC_URL --output json | jq  -r .data | base64 -d | jq)"
    echo $OUTPUT
